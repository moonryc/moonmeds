...
const NotificationsParent = () => {
  ...
  useEffect(() => {
    let errorAlerts = notifications.filter(
      (alert) => alert.severity === "error"
    );
    let warningAlerts = notifications.filter(
      (alert) => alert.severity === "warning"
    );
    let infoAlerts = notifications.filter((alert) => alert.severity === "info");
    let successAlerts = notifications.filter(
    ...
    <Box>
      {priorityNotification != null ? (
        ...
        >
          <Alert
            onClose={handleClose}
            severity={priorityNotification.severity}
            sx={{ width: "100%" }}
          >
            {priorityNotification.severity === "error" ? (
              <AlertTitle>Error</AlertTitle>
            ) : (
              <></>
            )}
            {priorityNotification.severity === "warning" ? (
